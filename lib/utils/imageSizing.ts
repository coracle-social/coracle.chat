...
/**
 * Get responsive sizing config based on screen width
 */
export const getResponsiveSizingConfig = (): ImageSizingConfig => {
  const screenWidth = Dimensions.get('window').width;

  if (screenWidth < 768) return ImageSizingPresets.responsive.mobile;
  if (screenWidth < 1024) return ImageSizingPresets.responsive.tablet;
  return ImageSizingPresets.responsive.desktop;
};
